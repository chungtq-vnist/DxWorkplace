...
import com.example.test.dxworkspace.databinding.FragmentDashboardWarehouseWasteBinding
...
class ReportWasteWarehouseFragment : BaseFragment<FragmentDashboardWarehouseWasteBinding>() {
    override fun getLayoutId(): Int {
        return R.layout.fragment_dashboard_warehouse_waste
    }
    ...
    fun setDataChart() {
        if(dataChart.size != 0) {
            ...
            dataChart.forEachIndexed { index, warehouseReportModel ->
                barEntriesBegin.add(BarEntry((index + 1).toFloat() , warehouseReportModel.wasteBeginningValue.toFloat()))
            }

            val barEntriesEnd = mutableListOf<BarEntry>()
            dataChart.forEachIndexed { index, warehouseReportModel ->
                barEntriesEnd.add(BarEntry((index + 1).toFloat() , warehouseReportModel.wasteEndingValue.toFloat()))
            }

            val barEntriesImport = mutableListOf<BarEntry>()
            dataChart.forEachIndexed { index, warehouseReportModel ->
                barEntriesImport.add(BarEntry((index + 1).toFloat() , warehouseReportModel.wasteImportValue.toFloat()))
            }

            val barEntriesExport = mutableListOf<BarEntry>()
            dataChart.forEachIndexed { index, warehouseReportModel ->
                barEntriesExport.add(BarEntry((index + 1).toFloat() , warehouseReportModel.wasteExportValue.toFloat()))
            }
//
//            val barEntriesBegin = mutableListOf<BarEntry>()
...
//            barEntriesExport.add(BarEntry(2f, (120000).toFloat()))
            ...
            chart.invalidate()
            adapter.items = dataChart[dataChart.size - 1].listWaste?.toMutableList() ?: mutableListOf()
        } else {
