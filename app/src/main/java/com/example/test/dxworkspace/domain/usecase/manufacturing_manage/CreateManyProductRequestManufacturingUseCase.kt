...
class CreateManyProductRequestManufacturingUseCase @Inject constructor(
    val manufacturingManagerRepository: ManufacturingManagerRepository,
    val configRepository: ConfigRepository
) : UseCase<Boolean, List<ParamCreateProductRequest>>() {
    override suspend fun run(params: List<ParamCreateProductRequest>): Either<Failure, Boolean> {
        return manufacturingManagerRepository.createManyRequest(
            params
        )
    }
