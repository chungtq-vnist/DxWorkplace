...
class GetListProductRequestByIdsUseCase @Inject constructor(
    val manufacturingManagerRepository: ManufacturingManagerRepository,
    val configRepository: ConfigRepository
) : UseCase<List<ProductRequestManagementModel>, List<String>>() {
    override suspend fun run(params: List<String>): Either<Failure, List<ProductRequestManagementModel>> {
        return manufacturingManagerRepository.getListRequestByIds(
            params
        )
    }
