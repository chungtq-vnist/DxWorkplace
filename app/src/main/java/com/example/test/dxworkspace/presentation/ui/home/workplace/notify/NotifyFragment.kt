...
//                allNotify = it?.docs ?: listOf()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.apply {
            ...
            rlTask.setOnClickListener {
                postNormal(EventNextHome(NotifyByTypeFragment::class.java, bundleOf(Pair("TYPE",1))))
            }
            rlAsset.setOnClickListener {
                postNormal(EventNextHome(NotifyByTypeFragment::class.java, bundleOf(Pair("TYPE",2))))
            }
            rlKPI.setOnClickListener {
                postNormal(EventNextHome(NotifyByTypeFragment::class.java, bundleOf(Pair("TYPE",3))))
            }
            rlProduct.setOnClickListener {
                postNormal(EventNextHome(NotifyByTypeFragment::class.java, bundleOf(Pair("TYPE",5))))
            }
            rlProcedure.setOnClickListener {
                postNormal(EventNextHome(NotifyByTypeFragment::class.java, bundleOf(Pair("TYPE",6))))
            }
        }
        getNotifications()
    }
